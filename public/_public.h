...
// ...:填充到格式控制描述fmt中的参数
// 返回值:格式化后的字符串的长度,程序员一般不关心返回值
// 注意:windows和linux平台下的snprintf函数的第三个参数n的用法略有不同假设格式化后的字符串的长度
...
// times:执行删除文件的次数,缺省是1,建议不要超过3,从实际应用的经验看来,如果删除文件第1次不成功,再尝试2次是可以的,更多次就意义不大了还有,如果执行删除失败,usleep(100000)后再重试
...
// times:执行重命名文件的次数,缺省是1,建议不要超过3,从实际应用的经验看来,如果重命名文件第1次不成功,再尝试2次是可以的,更多次就意义不大了还有,如果执行重命名失败,usleep(100000)后再重试
