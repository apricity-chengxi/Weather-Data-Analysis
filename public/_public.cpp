...
// 2次是可以的,更多就意义不大了还有,如果执行删除失败,usleep(100000)后再重试
...
// 试2次是可以的,更多次就意义不大了还有,如果执行重命名失败,usleep(100000)后再重试
...
bool CTcpClient::ConnectToServer(const char *ip, const int port)
{
    ...
    // 如果send到一个disconnected socket上,内核就会发出SIGPIPE信号这个信号
    // 的缺省处理方法是终止进程,大多数时候这都不是我们期望的我们重新定义这
